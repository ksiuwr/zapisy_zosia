...
def generate_meals(preference):
    ...
    if preference.dinner_1:
        result += u'{ Czw - obiad, 18:00-20:00 ' + str(is_vegetarian(preference))  +'}'
    else:
        result += u'{}'
    if preference.breakfast_2:
        result += u'{ Pią - śniadanie, 8:00-10:30 '+ str(is_vegetarian(preference))  +'}'
    else:
        result += u'{}'
    if preference.dinner_2:
        result += u'{ Pią - obiad, 17:00-19:00 '+ str(is_vegetarian(preference))  +'}'
    else:
        result += u'{}'
    if preference.breakfast_3:
        result += u'{ Sob - śniadanie, 8:00-10:30 '+ str(is_vegetarian(preference))  +'}'
    else:
        result += u'{}'
    if preference.dinner_3:
        result += u'{ Sob - obiad, 17:00-19:00 '+  str(is_vegetarian(preference))  +'}'
    else:
        result += u'{}'
    if preference.breakfast_4:
        result += u'{ Nie - śniadanie, 8:00-10:30 '+ str(is_vegetarian(preference)) + '}'
    else:
        result += u'{}'
